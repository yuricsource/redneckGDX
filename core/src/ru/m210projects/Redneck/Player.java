...
import static ru.m210projects.Redneck.Factory.RRGameProcessor.MAXHORIZ;
import static ru.m210projects.Redneck.Gamedef.furthestangle;
...
import static ru.m210projects.Redneck.Spawn.guts;
...
import static ru.m210projects.Redneck.Weapons.weaponprocess;
...
public class Player {
    ...
    public static void getMoto(PlayerStruct p, int i) {
        if (!p.OnMotorcycle) {
            ...
            if (sp != null) {
                ...
                p.posy = sp.getY();
                p.ang = sp.getAng();
                p.ammo_amount[MOTO_WEAPON] = sp.getOwner();
                p.UpdatePlayerLoc();
        ...
        if (Sound[186].getSoundOwnerCount() == 0) {
            spritesound(186, p.i);
        }
    }
    ...
    public static void leaveMoto(PlayerStruct p) {
        if (p.OnMotorcycle) {
            if (Sound[188].getSoundOwnerCount() > 0) {
                stopsound(Sound[188].getSoundOwnerCount(), p.i);
            }
            if (Sound[187].getSoundOwnerCount() > 0) {
                stopsound(Sound[187].getSoundOwnerCount(), p.i);
            }
            if (Sound[186].getSoundOwnerCount() > 0) {
                stopsound(Sound[186].getSoundOwnerCount(), p.i);
            }
            if (Sound[214].getSoundOwnerCount() > 0) {
                stopsound(Sound[214].getSoundOwnerCount(), p.i);
            }
            if (Sound[42].getSoundOwnerCount() == 0) {
                spritesound(42, p.i);
            }
            ...
            p.posxv = 0;
            p.posyv = 0;
            p.posxv -= EngineUtils.sin(((short) p.ang + 512) & kAngleMask) << 7;
            p.posyv -= EngineUtils.sin(((short) p.ang) & kAngleMask) << 7;
            int j = spawn(p.i, MOTORCYCLE);
            Sprite sp = boardService.getSprite(j);
            if (sp != null) {
                sp.setAng((short) p.ang);
                sp.setXvel(sp.getXvel() + (EngineUtils.cos(((short) p.ang) & kAngleMask) << 7));
                sp.setYvel(sp.getYvel() + (EngineUtils.sin(((short) p.ang) & kAngleMask) << 7));
                sp.setOwner((short) p.ammo_amount[MOTO_WEAPON]);
            }
        }
    }

    public static void getBoard(PlayerStruct p, int i) {
        if (!p.OnBoat) {
            Sprite sp = boardService.getSprite(i);
            if (sp != null) {
                p.posx = sp.getX();
                p.posy = sp.getY();
                p.ang = sp.getAng();
                p.ammo_amount[BOAT_WEAPON] = sp.getOwner();
                p.UpdatePlayerLoc();
                engine.deletesprite(i);
            }
            over_shoulder_on = 0;
    ...
    public static void leaveBoard(PlayerStruct p) {
        if (p.OnBoat) {
            ...
            if (sp != null) {
                ...
                sp.setXvel(sp.getXvel() + (EngineUtils.cos(((short) p.ang) & kAngleMask) << 7));
                sp.setYvel(sp.getYvel() + (EngineUtils.sin(((short) p.ang) & kAngleMask) << 7));
                sp.setOwner((short) p.ammo_amount[BOAT_WEAPON]);
            }
        }
    }
    ...
    public static void hitawall(PlayerStruct p)
    {
        if (IsOriginalDemo()) {
        ...
        FindDistance2D(pHitInfo.hitx - p.posx, pHitInfo.hity - p.posy);
    }
    ...
    public static boolean doincrements(PlayerStruct p) {
        ...
        if (BellTime > 0 && --BellTime == 0) {
            Sprite bellSprite = boardService.getSprite(word_119BE0);
            if (bellSprite != null) {
                bellSprite.setPicnum(bellSprite.getPicnum() + 1);
            }
        }
        ...
        if (p.alcohol_amount == 100) {
            p.drunk = 1;
            if (Sound[420].getSoundOwnerCount() == 0) {
                spritesound(420, p.i);
            }
        ...
        if (p.gut_amount >= 31) {
            if (engine.krand() < p.alcohol_amount) {
                ...
                if (numplayers < 2) {
                    p.field_290 = 0x4000;
                    sub_64EF0(snum);
                    p.posxv += mulscale(16, EngineUtils.sin(((int) p.ang + 512) & kAngleMask), 16);
                    p.posyv += mulscale(16, EngineUtils.sin((int) p.ang & kAngleMask), 16);
                }
                p.gut_amount -= 4;
                if (p.gut_amount < 0) {
                    p.gut_amount = 0;
    ...
    private static int motorcycleprocessinput(int snum) {
        ...
        if ((sb_snum & 1) != 0) {
            ...
            if (p.on_ground) {
                if (p.CarSpeed == 0 && var != 0) {
                ...
                } else if (p.CarSpeed != 0 || Sound[214].getSoundOwnerCount() != 0) {
                    if (p.CarSpeed < 50 || Sound[188].getSoundOwnerCount() != 0) {
                        if (Sound[188].getSoundOwnerCount() == 0 && Sound[214].getSoundOwnerCount() == 0) {
                            spritesound(188, pi);
                        }
                    } else {
                        spritesound(188, pi);
                    ...
                    if (Sound[187].getSoundOwnerCount() > 0) {
                        stopsound(Sound[187].getSoundOwnerCount(), pi);
                    }
                    spritesound(214, pi);
                }
            }
            ...
            var2 = 0;
            if (Sound[214].getSoundOwnerCount() > 0) {
                stopsound(Sound[214].getSoundOwnerCount(), pi);
                if (Sound[189].getSoundOwnerCount() == 0) {
                    spritesound(189, pi);
                }
            }
            if (Sound[188].getSoundOwnerCount() > 0) {
                stopsound(Sound[188].getSoundOwnerCount(), pi);
                if (Sound[189].getSoundOwnerCount() == 0) {
                    spritesound(189, pi);
                }
            }
            if (Sound[189].getSoundOwnerCount() == 0 && Sound[187].getSoundOwnerCount() == 0) {
                spritesound(187, pi);
            }
        }
        ...
        if ((sb_snum & 16) != 0 || sync[snum].avel < 0) {
            var4 = 1;
            sb_snum &= ~16;
        }

        if ((sb_snum & 64) != 0 || sync[snum].avel > 0) {
            var5 = 1;
        }

        if (p.alcohol_amount > 88 && p.CarVar2 == 0 && p.alcohol_amount <= 99) { // #GDX 07.07.2024
        ...
        } else if (p.alcohol_amount > 99 && p.CarVar2 == 0) {
        ...
        if (p.TurbCount != 0) {
            ...
            if (p.VBumpTarget >= p.VBumpNow) {
                ...
                p.horiz = (float) ((p.VBumpNow) / 3 + 100);
            }
        } else {
            if (p.CarVar3 != 0) {
                p.VBumpNow += 6;
        ...
        if (p.CarSpeed >= 20 && p.on_ground && (var4 != 0 || var5 != 0)) {
            ...
            if (p.CarVar5 != 0 || p.CarVar4 != 0 || p.NotOnWater == 0) {
                ...
                if (p.CarVar6 != 0) {
                    p.posxv += (speed >> 5) * 16 * EngineUtils.cos((angvel) & kAngleMask);
                    p.posyv += (speed >> 5) * 16 * EngineUtils.sin(angvel & kAngleMask);
                    p.ang = ((short) p.ang - (dang >> 2)) & kAngleMask;
                } else {
                    p.posxv += (speed >> 7) * 16 * EngineUtils.cos((angvel) & kAngleMask);
                    p.posyv += (speed >> 7) * 16 * EngineUtils.sin(angvel & kAngleMask);
                    p.ang = ((short) p.ang - (dang >> 6)) & kAngleMask;
                }
            } else if (p.CarVar6 != 0) {
                p.posxv += (p.CarSpeed >> 5) * 16 * EngineUtils.cos((angvel) & kAngleMask);
                p.posyv += (p.CarSpeed >> 5) * 16 * EngineUtils.sin(angvel & kAngleMask);
                p.ang = ((short) p.ang - (dang >> 4)) & kAngleMask;
                if (Sound[220].getSoundOwnerCount() == 0) {
                    spritesound(220, p.i);
                }
            } else {
                p.posxv += (p.CarSpeed >> 7) * 16 * EngineUtils.cos((angvel) & kAngleMask);
                p.posyv += (p.CarSpeed >> 7) * 16 * EngineUtils.sin(angvel & kAngleMask);
                p.ang = ((short) p.ang - (dang >> 4)) & kAngleMask;
            }
    ...
    private static int boatprocessinput(int snum) {
        final PlayerStruct p = ps[snum];
        int pi = p.i;
        ...
        Sector psec = boardService.getSector(psect);
        if (boardService.getSector(psp.getSectnum()) == null || psec == null) {
            return sb_snum;
        }

        if (p.NotOnWater != 0) {
            if (p.CarSpeed <= 0) {
                if (Sound[87].getSoundOwnerCount() == 0) {
                    spritesound(87, pi);
                }
            } else if (Sound[88].getSoundOwnerCount() == 0) {
                spritesound(88, pi);
            }
        }
        if (p.CarSpeed < 0) {
        ...
        boolean var1 = false, var2, var3 = false, var4 = false;
        ...
        if ((sb_snum & 1) != 0) {
            var2 = true;
            sb_snum &= ~1;
            if (p.CarSpeed != 0 || Sound[89].getSoundOwnerCount() != 0) {
                if (p.CarSpeed < 50 || Sound[88].getSoundOwnerCount() != 0) {
                    if (Sound[88].getSoundOwnerCount() == 0 && Sound[89].getSoundOwnerCount() == 0) {
                        spritesound(88, pi);
                    }
                } else {
                    spritesound(88, pi);
                ...
                if (Sound[87].getSoundOwnerCount() > 0) {
                    stopsound(Sound[87].getSoundOwnerCount(), pi);
                }
                spritesound(89, pi);
            }
        } else {
            var2 = false;
            if (Sound[89].getSoundOwnerCount() > 0) {
                stopsound(Sound[89].getSoundOwnerCount(), pi);
                if (Sound[90].getSoundOwnerCount() == 0) {
                    spritesound(90, pi);
                }
            }
            if (Sound[88].getSoundOwnerCount() > 0) {
                stopsound(Sound[88].getSoundOwnerCount(), pi);
                if (Sound[90].getSoundOwnerCount() == 0) {
                    spritesound(90, pi);
                }
            }
            if (Sound[90].getSoundOwnerCount() == 0 && Sound[87].getSoundOwnerCount() == 0) {
                spritesound(87, pi);
            }
        }
        if ((sb_snum & 2) != 0) {
        ...
        if ((sb_snum & 16) != 0 || sync[snum].avel < 0) {
            var5 = true;
            sb_snum &= ~16;
            if (Sound[91].getSoundOwnerCount() == 0 && p.CarSpeed > 30 && p.NotOnWater == 0) {
                spritesound(91, pi);
            }
        }
        ...
        if (p.on_ground) {
            if (var1) {
                if (p.CarSpeed > 25) {
                    ...
                    ++p.CarSpeed;
                    if (Sound[182].getSoundOwnerCount() == 0) {
                        spritesound(182, pi);
                    }
                }
        ...
        if (p.TurbCount != 0) {
            ...
            if (p.VBumpTarget >= p.VBumpNow) {
                ...
                p.horiz = (float) ((p.VBumpNow) / 3 + 100); // FIXME demo
            }
        } else {
            if (p.CarVar3 != 0) {
                p.VBumpNow += 6;
        ...
        if (p.CarSpeed > 0 && p.on_ground && (var5 || var6)) {
            ...
            int speed = 4 * p.CarSpeed;
            if (p.CarVar6 != 0) {
                p.posxv += (speed >> 6) * 16 * EngineUtils.cos((angvel) & 0x7FF);
                p.posyv += (speed >> 6) * 16 * EngineUtils.sin(angvel & 0x7FF);
                p.ang = ((short) p.ang - (dang >> 5)) & kAngleMask;
            } else {
                p.posxv += (speed >> 7) * 16 * EngineUtils.cos((angvel) & 0x7FF);
                p.posyv += (speed >> 7) * 16 * EngineUtils.sin(angvel & 0x7FF);
                p.ang = ((short) p.ang - (dang >> 6)) & kAngleMask;
            }
        }

        if (p.NotOnWater != 0 && p.CarSpeed > 50) {
            p.CarSpeed -= (short) (p.CarSpeed >> 1);
        }
    ...
    public static void processinput(int snum) {
        ...
        if (p.OnMotorcycle) {
            boolean left = (sb_snum & 16) != 0 || p.CarSpeed >= 80 && sync[snum].avel < 0;
            boolean right = (sb_snum & 64) != 0 || p.CarSpeed >= 80 && sync[snum].avel > 0;
            if (p.CarSpeed != 0 && p.on_ground) {
                if (left || p.CarVar2 < 0) {
        ...
        if (p.OnBoat) {
            boolean left = (sb_snum & 16) != 0 || sync[snum].avel < 0;
            boolean right = (sb_snum & 64) != 0 || sync[snum].avel > 0;
            if (p.CarSpeed != 0) {
                if (left || p.CarVar2 < 0) {
                    if (p.NotOnWater == 0) {
                        p.TiltStatus = BClipLow(p.TiltStatus - 1, -10);
        ...
        if (p.OnMotorcycle && psp.getExtra() > 0) {
            sb_snum = motorcycleprocessinput(snum);
        } else if (p.OnBoat && psp.getExtra() > 0) {
            sb_snum = boatprocessinput(snum);
        }

        int psectlotag = psec.getLotag();
        if (psectlotag == 867) {
            for (ListNode<Sprite> node = boardService.getSectNode(psect); node != null; node = node.getNext()) {
                Sprite sp = node.get();
                if (sp.getPicnum() == 380 && sp.getZ() - 2048 < p.posz) {
                    psectlotag = 2;
                }
            }
        ...
        boolean shrunk = (psp.getYrepeat() < 8);

        if (psp.getClipdist() == 64) {
        ...
        if (p.aim_mode == 0 && p.on_ground && psectlotag != 2 && (psec.getFloorstat() & 2) != 0) {
            int x, y;
            if (IsOriginalDemo()) {
                x = p.posx + (EngineUtils.cos(((int) p.ang) & 2047) >> 5);
        ...
        if ((hz & kHitTypeMask) == kHitSprite) {
            ...
            if (pHitSprite != null) {
                if (pHitSprite.getStatnum() == 1 && pHitSprite.getExtra() >= 0) {
                    cz = p.truecz;
                }

                if (pHitSprite.getPicnum() == 3587) {
                    if (p.field_280 != 0) {
                        p.field_280--;
                    } else {
                        p.field_280 = 10;
        ...
        if ((lz & kHitTypeMask) == kHitSprite) {
            int j = lz & kHitIndexMask;
            Sprite pHitSprite = boardService.getSprite(j);
            ...
            if (p.OnMotorcycle && badguy(pHitSprite)) {
                hittype[j].picnum = 7170;
                hittype[j].extra = (p.CarSpeed >> 1) + 2;
                p.CarSpeed -= (short) (p.CarSpeed >> 4);
            }
            if (p.OnBoat) {
                if (badguy(pHitSprite)) {
                    hittype[j].picnum = 7170;
                    hittype[j].extra = (p.CarSpeed >> 1) + 2;
                    p.CarSpeed -= (short) (p.CarSpeed >> 4);
                }
            } else if (badguy(pHitSprite) && pHitSprite.getXrepeat() > 24 && klabs(psp.getZ() - pHitSprite.getZ()) < (84 << 8)) {
                j = EngineUtils.getAngle(pHitSprite.getX() - p.posx, pHitSprite.getY() - p.posy);
                p.posxv -= EngineUtils.sin((j + 512) & 2047) << 4;
            ...
            if (pHitSprite != null) {
                ...
                } else if ((pHitSprite.getPicnum() == 1098 || pHitSprite.getPicnum() == 2121) && (sb_snum & 2) != 0
                        && Sound[436].getSoundOwnerCount() == 0 && !p.OnMotorcycle) {
                    spritesound(436, p.i);
            ...
            if (p.dead_flag == 0) {
                ...
                if (ud.multimode > 1 && (psp.getPal() != 1 || (psp.getCstat() & 32768) != 0)) {
                    if (p.frag_ps != snum) {
                        ...
                        if (ud.user_name[p.frag_ps] != null && ud.user_name[p.frag_ps].codePointAt(0) != 0) {
                            if (snum == screenpeek) {
        ...
        if (!SHOOTINCODE) {
            ...
            if (p.on_crane < 0) {
                ...
                if (psectlotag == 17 || psectlotag == 18) {
                    if (getanimationgoal(boardService.getSector(p.cursectnum), FLOORZ) < 0) {
                        stopsound(432, pi);
                    } else if (Sound[432].getSoundOwnerCount() == 0) {
                        spritesound(432, pi);
                    }
                }
                ...
                if (p.isSea) {
                    ...
                    if (p.SeaSick != 0) {
                        p.pyoff = EngineUtils.sin(p.pycount) >> 2;
                    } else {
                        ...
                        if (!shrunk && truefdist <= PHEIGHT) {
                            if (p.on_ground) {
                                ...
                                if (psec2 != null && psec2.getFloorpicnum() == FLOORSLIME) {
                                    p.footprintpal = 8;
                                    p.footprintshade = 0;
                                } else if (psec2 != null && psec2.getFloorpicnum() != 7756
                                        && psec2.getFloorpicnum() != 7888) {
                                    p.footprintpal = 0;
                                    p.footprintshade = 0;
                                } else {
                                    p.footprintpal = 0;
                        ...
                        if (!p.OnMotorcycle && p.footprintcount > 0 && p.on_ground) {
                            if (psec2 != null && (psec2.getFloorstat() & 2) != 2) {
                                ...
                                for (; node != null; node = node.getNext()) {
                                    Sprite sp = node.get();
                                    if (sp.getPicnum() == FOOTPRINTS || sp.getPicnum() == FOOTPRINTS2
                                            || sp.getPicnum() == FOOTPRINTS3 || sp.getPicnum() == FOOTPRINTS4) {
                                        if (klabs(sp.getX() - p.posx) < 384) {
                                            if (klabs(sp.getY() - p.posy) < 384) {
                                                break;
                                            }
                                        }
                                    }
                        ...
                        if ((sb_snum & 3) == 0 && p.on_ground && (psec.getFloorstat() & 2) != 0
                            ...
                            if ((p.OnMotorcycle || p.OnBoat) && fz - (i << 9) > p.posz) {
                                if (p.CarOnGround) {
                                    ...
                                    if (Sound[189].getSoundOwnerCount() == 0 && Sound[190].getSoundOwnerCount() == 0) {
                                        spritesound(190, pi);
                                    }
                                }
                            } else {
                                ...
                                if (psec2 != null && psec2.getLotag() != 1) {
                                    ...
                                    if (p.falling_counter > 62) {
                                        ...
                                        if (p.poszv <= 2048) {
                                            ...
                                            if (Sound[190].getSoundOwnerCount() > 0) {
                                                stopsound(Sound[190].getSoundOwnerCount(), pi);
                                            }
                                            spritesound(191, pi);
                                            p.TurbCount = 12;
                                        } else {
                ...
                if (p.posxv != 0 || p.posyv != 0 || sync[snum].fvel != 0 || sync[snum].svel != 0) {
                    ...
                    if (currentGame.getCON().type != RRRA && (p.curr_weapon == KNEE_WEAPON && p.kickback_pic > 10 && p.on_ground || p.on_ground && (sb_snum & 2) != 0)) {
            ...
            if ((j & kHitTypeMask) == kHitWall) {
                final int nwall = j & kHitIndexMask;
                Wall hwp = boardService.getWall(nwall);
                if (hwp != null) {
                    if (p.OnMotorcycle) {
                        int dang = klabs((short) p.ang - hwp.getWallAngle());
                        if ((engine.krand() & 1) == 1) {
                        ...
                        if (dang >= 441 && dang <= 581) {
                            ...
                            p.CarSpeed = 0;
                            if (Sound[238].getSoundOwnerCount() == 0) {
                                spritesound(238, p.i);
                            }
                        } else if (dang >= 311 && dang <= 711) {
                            damage = (p.CarSpeed * p.CarSpeed) >> 11;
                            p.CarSpeed -= (short) ((p.CarSpeed >> 2) + (p.CarSpeed >> 1));
                            if (Sound[238].getSoundOwnerCount() == 0) {
                                spritesound(238, p.i);
                            }
                            ...
                            p.CarSpeed -= (short) (p.CarSpeed >> 3);
                            if (Sound[240].getSoundOwnerCount() == 0) {
                                spritesound(240, p.i);
                            }
                        }
                        psp.setExtra(psp.getExtra() - damage);
                        if (psp.getExtra() > 0) {
                            if (damage != 0) {
                                spritesound(200, pi);
                            }
                        ...
                        if (dang >= 441 && dang <= 581) {
                            ...
                            if (psectlotag == 1 && Sound[178].getSoundOwnerCount() == 0) {
                                spritesound(178, p.i);
                            }
                        } else if (dang >= 311 && dang <= 711) {
                            p.CarSpeed -= (short) (((p.CarSpeed >> 1) + (p.CarSpeed >> 2)) >> 3);
                            if (psectlotag == 1 && Sound[179].getSoundOwnerCount() == 0) {
                                spritesound(179, p.i);
                            }
                        } else if (dang >= 111 && dang <= 911) {
                            p.CarSpeed -= (short) (p.CarSpeed >> 4);
                            if (psectlotag == 1 && Sound[180].getSoundOwnerCount() == 0) {
                                spritesound(180, p.i);
                            }
                        } else {
                            p.CarSpeed -= (short) (p.CarSpeed >> 6);
                            if (Sound[181].getSoundOwnerCount() == 0) {
                                spritesound(181, p.i);
                            }
                        }
                    } else if (hwp.getLotag() >= 40 && hwp.getLotag() <= 44) {
                        if (hwp.getLotag() < 44) {
                            pushwall(hwp, psec, snum);
                        }
            ...
            if ((j & kHitTypeMask) == kHitSprite) {
                final int nspr = j & kHitIndexMask;
                Sprite hsp = boardService.getSprite(nspr);
                if (hsp != null) {
                    if (p.OnMotorcycle) {
                        if (badguy(hsp) || hsp.getPicnum() == APLAYER) {
                            if (hsp.getPicnum() == APLAYER) {
                                ...
                                movesprite((short) nspr, EngineUtils.sin(((short) p.ang + 20 * p.TiltStatus + 512) & 0x7FF) >> 8,
                                        EngineUtils.sin(((short) p.ang + 20 * p.TiltStatus) & 0x7FF) >> 8, hsp.getZvel(),
                                        CLIPMASK0);
                            }
                            hittype[nspr].picnum = 7170;
                            hittype[nspr].extra = p.CarSpeed >> 1;
                            p.CarSpeed -= (short) (p.CarSpeed >> 2);
                            p.TurbCount = 6;
                        } else if ((hsp.getPicnum() == 2431 || hsp.getPicnum() == 2443
                                || hsp.getPicnum() == 2451 || hsp.getPicnum() == 2455) && hsp.getPicnum() != 4
                                && p.CarSpeed > 45) {
                            spritesound(69, nspr);
                            if (hsp.getPicnum() != 2431 && hsp.getPicnum() != 2451) {
                                guts(hsp, 2465, 3, myconnectindex);
                            } else {
                                if (hsp.getLotag() != 0) {
                                    for (int l = 0; l < boardService.getSpriteCount(); ++l) {
                                        Sprite sp = boardService.getSprite(l);
                                        if (sp != null) {
                                            if ((sp.getPicnum() == 2431 || sp.getPicnum() == 2451) && sp.getPal() == 4
                                                    && hsp.getLotag() == sp.getLotag()) {
                                                sp.setXrepeat(0);
                                                sp.setYrepeat(0);
                                            }
                                        }
                                ...
                                guts(hsp, 2460, 12, myconnectindex);
                                guts(hsp, 2465, 3, myconnectindex);
                            }
                            guts(hsp, 2465, 3, myconnectindex);
                            hsp.setXrepeat(0);
                        ...
                        if (badguy(hsp) || hsp.getPicnum() == APLAYER) {
                            if (hsp.getPicnum() == APLAYER) {
                                ...
                                movesprite((short) nspr, EngineUtils.sin(((short) p.ang + 20 * p.TiltStatus + 512) & 0x7FF) >> 9,
                                        EngineUtils.sin(((short) p.ang + 20 * p.TiltStatus) & 0x7FF) >> 9, hsp.getZvel(),
                                        CLIPMASK0);
                            }
                            hittype[nspr].picnum = 7170;
                    ...
                    } else if (hsp.getPicnum() == 3410) {
                        quickkill(p);
                        spritesound(446, pi);
                    } else if (hsp.getPicnum() == 2443 && hsp.getPal() == 19) {
                        hsp.setPal(0);
                        p.DrugMode = 5;
            ...
            if (psec2 != null && truefdist < PHEIGHT && p.on_ground && psectlotag != 1 && !shrunk && psec2.getLotag() == 1) {
                if (Sound[DUKE_ONWATER].getSoundOwnerCount() == 0 && !p.OnBoat && !p.OnMotorcycle && psec2.getHitag() != 321) {
                    spritesound(DUKE_ONWATER, pi);
                }
            }
            ...
            if (!ud.clipping) {
                if (klabs(hittype[pi].floorz - hittype[pi].ceilingz) < (48 << 8) || j != 0) {
                    ...
                    if (pssec != null && (pssec.getLotag() & 0x8000) == 0
                        ...
                        activatebysector(psp.getSectnum(), snum);
                    }
                    if (j != 0) {
                        quickkill(p);
                        return;
                ...
                if (psec != null && psec.getFloorz() - 3072 < psec.getCeilingz()) {
                    quickkill(p);
                    return;
                }
            }
    ...
    public static boolean checkaccess(int sectnum, int snum) {
        ...
        if (sec.getFiller() > 3) {
            sec.setFiller(sec.getFiller() - 3);
        }

        if (ps[snum].gotkey[sec.getFiller()] != 0) {
            sec.setFiller(0);
            return true;
        }
    ...
    public static void computergetinput(int snum, Input syn) {
        ...
        Sprite goalSprite = boardService.getSprite(ps[goalplayer[snum]].i);
        ...
        if ((ps[goalplayer[snum]].dead_flag == 0)
            ...
            if ((p.curr_weapon == CROSSBOW_WEAPON) && ((engine.krand() & 7)) == 0) {
                syn.bits &= ~(1 << 2);
            }
            ...
            if (p.curr_weapon == DYNAMITE_WEAPON) {
                engine.hitscan(x1, y1, z1 - PHEIGHT, damysect, EngineUtils.cos((damyang) & 2047),
                        EngineUtils.sin(damyang & 2047), (100 - (int) p.horiz - p.horizoff) * 32, pHitInfo, CLIPMASK1);
            ...
            if (fightdist != 0) {
                ...
                dx = EngineUtils.sin((goalSprite.getAng() + 512) & 2047);
                    ...
                    if ((dashow2dsector[j >> 3] & (1 << (j & 7))) == 0) {
                        ...
                        if (j == endsect) {
                            ...
                            for (ListNode<Wall> wn2 = startsec.getWallNode(); wn2 != null; wn2 = wn2.getNext()) {
                                ...
                                // if (dx*(y1-boardService.getWall(i).y) <= dy*(x1-boardService.getWall(i).x))
                                // if (dx*(y2-boardService.getWall(i).y) >= dy*(x2-boardService.getWall(i).x))
        ...
        if (moveHit != 0) {
            ...
            moveHit = EngineUtils.sin((j << 6) & 2047);
            moveHit += EngineUtils.sin(((j + 4245) << 5) & 2047);
            moveHit += EngineUtils.sin(((j + 6745) << 4) & 2047);
            moveHit += EngineUtils.sin(((j + 15685) << 3) & 2047);
            syn.fvel += (short) ((EngineUtils.cos((daang + 512) & 2047) * moveHit) >> 17);
            syn.svel += (short) ((EngineUtils.sin((daang + 512) & 2047) * moveHit) >> 17);
