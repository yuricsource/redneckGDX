...
public class Player {
	...
	public static void quickkill(PlayerStruct p)
	{
	    ...
	    if(!ud.god) guts(sprite[p.i],JIBS6,8,myconnectindex);
	    return;
	}
	...
	public static int hits(int i)
	{
	    ...
	    if(spr.picnum == APLAYER) zoff = (40<<8);

	    engine.hitscan(spr.x,spr.y,spr.z-zoff,spr.sectnum,
	        sintable[(spr.ang+512)&2047],
	        sintable[spr.ang&2047],
	...
	{
		if(IsOriginalDemo()) {
			 engine.hitscan(p.posx,p.posy,p.posz,p.cursectnum,
					 sintable[((int)p.ang+512)&2047],
					 sintable[(int)p.ang&2047],
			        0, pHitInfo,CLIPMASK0);
			 
		} else {
			engine.hitscan(p.posx,p.posy,p.posz,p.cursectnum,
	    		(int)BCosAngle(BClampAngle(p.ang)),
	    		(int)BSinAngle(BClampAngle(p.ang)),
	        0, pHitInfo,CLIPMASK0);
		}
	...
	public static boolean doincrements(PlayerStruct p)
	{
	    ...
	    if ( p.gut_amount >= 31 )
	    {
	    	if(engine.krand() < p.alcohol_amount)
	    	{
	    		...
	    		{
	    			case 0: spritesound(404, p.i); break;
	    			case 1: spritesound(422, p.i); break;
	    			case 2: spritesound(423, p.i); break;
	    			case 3: spritesound(424, p.i); break;
	    		}
	    		if ( numplayers < 2 )
	    		{
	    	        p.field_290 = 0x4000;
	    ...
	    if(p.access_incs != 0 && sprite[p.i].pal != 1)
	    {
	        ...
	        if(p.access_incs == 12)
	        {
	            if(p.access_spritenum >= 0)
	            {
	                ...
	                {
	                    case 0:p.gotkey[1] = 1;break;
	                    case 21:p.gotkey[2] = 1;break;
	                    case 23:p.gotkey[3] = 1;break;
	                }
	        ...
	        if(p.knuckle_incs==9)
	        {
	        	...
	        	if ( ud.volume_number == 0 )	
	        	{
	        		switch( ud.level_number )
        			{
	    				...
	    				case 2: snd = 77; break;
	    				case 3: snd = 80; break;
	    				case 4: snd = 102; break;
	    				case 5: snd = 103; break;
	    				case 6: snd = 104; break;
        			}
	        	}
	...
	public static void processinput(int snum)
	{
	    ...
	    else engine.getzrange(p.posx,p.posy,p.posz,psect,4,CLIPMASK0);
	        ...
	        if(p.dead_flag == 0)
	        {
	            ...
	            if( ud.multimode > 1 && ( s.pal != 1 || (s.cstat&32768) != 0) )
	            {
	                if(p.frag_ps != snum)
	                {
	                    ...
	                    if( ud.user_name != null && ud.user_name[p.frag_ps] != null &&ud.user_name[p.frag_ps].codePointAt(0) != 0)
	                    {
	                        if(snum == screenpeek)
	                        {
	    ...
	    if(!SHOOTINCODE) 
	    {
		    ...
		    if(p.on_crane < 0) 
		    {
			    ...
			    s.xvel = (short) engine.ksqrt( (p.posx-p.bobposx)*(p.posx-p.bobposx)+(p.posy-p.bobposy)*(p.posy-p.bobposy));
			    if(p.on_ground) p.bobcounter += sprite[p.i].xvel>>1;
		
			    if( !ud.clipping && ( sector[p.cursectnum].floorpicnum == MIRROR || p.cursectnum < 0 || p.cursectnum >= MAXSECTORS) )
			    {
			        p.posx = p.oposx;
			        p.posy = p.oposy;
			    }
			    ...
			    else if( psectlotag != 2 )
			    {
			        ...
			        {
			            if( (sb_snum&3) == 0 && p.on_ground && (sector[psect].floorstat&2) != 0 && p.posz >= (fz-(i<<8)-(16<<8) ) )
			                p.posz = fz-(i<<8);
			            else
			            {
			                p.on_ground = false;
			    ...
			    {                         //ENGINE calculates angvel for you
			    	if(IsOriginalDemo()) {
				        ...
				        else p.angvel = (int) tempang*ksgn(doubvel);
...
//	        else k = 1;
	        ...
	        if(ud.clipping)
	        {
	            ...
	            j = engine.clipmove(p.posx,p.posy,
	                p.posz,p.cursectnum,
	                p.posxv,p.posyv,164,(4<<8),i,CLIPMASK0);
	        ...
	        if(!ud.clipping)
	        {
	            ...
	            {
	                if ( (sector[s.sectnum].lotag&0x8000) == 0 && ( isanunderoperator(sector[s.sectnum].lotag) ||
		    ...
		    if(p.knee_incs > 0)
		    {
		        ...
		        if(p.knee_incs > 15)
		        {
		            ...
		            if(p.actorsqu >= 0 && dist(sprite[pi],sprite[p.actorsqu]) < 1400 )
		            {
		                ...
		                else engine.deletesprite(p.actorsqu);
		            }
		            p.actorsqu = -1;
		        }
		        else if(p.actorsqu >= 0)
		            p.ang += getincangle(p.ang,engine.getangle(sprite[p.actorsqu].x-p.posx,sprite[p.actorsqu].y-p.posy)) / 4.0f;
		    }
	...
	public static int getspritescore(int snum, int dapicnum)
	{
	    switch(dapicnum)
	    {
	        ...
	        case 53: if (ps[snum].whishkey_amount < 100) return(100); else return(0);
	        case 54: if (ps[snum].shield_amount < 100) return(50); else return(0);
	...
	public static int[] goalsect = new int[MAXPLAYERS], goalwall = new int[MAXPLAYERS], goalsprite = new int[MAXPLAYERS];
	...
	public static void computergetinput(int snum, Input syn)
	{
	    ...
	    if ((goalplayer[snum] == snum) || (ps[goalplayer[snum]].dead_flag != 0))
	    {
	        ...
	        for(i=connecthead;i>=0;i=connectpoint2[i])
	            if (i != snum)
	            {
	                dist = engine.ksqrt((sprite[ps[i].i].x-x1)*(sprite[ps[i].i].x-x1)+(sprite[ps[i].i].y-y1)*(sprite[ps[i].i].y-y1));
	    ...
	    if (p.dead_flag != 0) syn.bits |= (1<<29);
	    ...
	    for(j=headspritestat[4];j>=0;j=nextspritestat[j])
	    {
	        switch (sprite[j].picnum)
	        {
	            ...
	            case ALIENBLAST: k = 4; break;
	            case CIRCLESAW: k = 16; break;
	            case CROSSBOW: k = 16; break;
	            default: k = 0; break;
	        }
	        if (k != 0)
	        {
	            ...
	            for(l=0;l<=8;l++)
	            {
	                ...
	                else
	                {
	                     ...
	                     mulscale(sprite[j].xvel,sintable[sprite[j].ang&2047], 14),
	                     sprite[j].zvel,
	                     pHitInfo,CLIPMASK1);
	         	...
	         	(engine.cansee(x1,y1,z1-(48<<8),damysect,x2,y2,z2-(48<<8),sprite[ps[goalplayer[snum]].i].sectnum)))	)
	    {
	        ...
	        dist = engine.ksqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)); if (dist == 0) dist = 1;
	    ...
	    if (goalsect[snum] < 0)
	    {
	        ...
	        for(splc=0,send=1;splc<send;splc++)
	        {
	            ...
	            for(i=startwall;i<endwall;i++)
	            {
	                ...
	                if ((engine.getceilzofslope((short)j,dx,dy) > engine.getflorzofslope((short)j,dx,dy)-(28<<8)) && ((sector[j].lotag < 15) || (sector[j].lotag > 22)))
	                    continue;
	                if (engine.getflorzofslope((short)j,dx,dy) < engine.getflorzofslope(searchsect[splc],dx,dy)-(72<<8))
	                    continue;
	                if ((dashow2dsector[j>>3]&(1<<(j&7))) == 0)
	                {
	                    ...
	                    if (j == endsect)
	                    {
	                        ...
	                        goalwall[snum] = k;
	                        daang = ((engine.getangle(wall[wall[k].point2].x-wall[k].x,wall[wall[k].point2].y-wall[k].y)+1536)&2047);
	    ...
	    if ((goalsect[snum] < 0) || (goalwall[snum] < 0))
	    {
	        ...
	        x2 = goalx[snum];
	        y2 = goaly[snum];
	        dist = engine.ksqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)); if (dist == 0) return;
	    ...
	    if (i != 0)
	    {
	        ...
	        if ((i&kHitTypeMask) == kHitWall) {
	            ...
	            {
	                if (engine.getflorzofslope(wall[nwall].nextsector,p.posx,p.posy) <= p.posz+(24<<8)) j |= 1;
	                if (engine.getceilzofslope(wall[nwall].nextsector,p.posx,p.posy) >= p.posz-(24<<8)) j |= 2;
	            }
	        }
	        if ((i&kHitTypeMask) == kHitSprite) j = 1;
	        if ((j&1) != 0) if (clipmovecount[snum] == 4) syn.bits |= (1<<0);
	        if ((j&2) != 0) syn.bits |= (1<<1);

	        //Strafe attack
	        daang = engine.getangle(x2-x1,y2-y1);
	        if ((i&kHitTypeMask) == kHitWall) {
	        	int nwall = i&kHitIndexMask;
	            daang = engine.getangle(wall[wall[nwall].point2].x-wall[nwall].x,wall[wall[nwall].point2].y-wall[nwall].y);
	        }
	        j = totalclock+snum*13468;
	        i = sintable[(j<<6)&2047];
	        i += sintable[((j+4245)<<5)&2047];
	    ...
	    if ((goalsect[snum] >= 0) && (goalwall[snum] >= 0))
	    {
	        ...
	        if ((goalwall[snum] >= 0) && (dist < 4096))
	            daang = ((engine.getangle(wall[wall[goalwall[snum]].point2].x-wall[goalwall[snum]].x,wall[wall[goalwall[snum]].point2].y-wall[goalwall[snum]].y)+1536)&2047);
	        syn.fvel += (x2-x1)*2047/dist;
	        syn.svel += (y2-y1)*2047/dist;
	        syn.avel = min(max(((BClampAngle(daang+1024-damyang))-1024) / 8.0f,-127),127);
	    }
