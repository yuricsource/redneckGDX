...
public class NetInfo {
    ...
    public void set(byte[] p, int ptr) {
        nGameType = LittleEndian.getUByte(p, ptr);
        ptr++;
        nEpisode = LittleEndian.getUByte(p, ptr);
        ptr++;
        nLevel = LittleEndian.getUByte(p, ptr);
        ptr++;
        nDifficulty = (byte) LittleEndian.getUByte(p, ptr);
        ptr++;
        nMonsters = LittleEndian.getUByte(p, ptr);
        ptr++;
        nRespawnMonsters = LittleEndian.getUByte(p, ptr);
        ptr++;
        nRespawnItem = LittleEndian.getUByte(p, ptr);
        ptr++;
        nRespawnInventory = LittleEndian.getUByte(p, ptr);
        ptr++;
        nMarkers = LittleEndian.getUByte(p, ptr);
        ptr++;
        nFriendlyFire = LittleEndian.getUByte(p, ptr);
    }
