...
public class Actors {
	...
	public static void hitradius(int i, int r, int hp1, int hp2, int hp3,
		...
		if (s.picnum != CROSSBOW || s.xrepeat >= 11) {
			...
			do {
				...
				if (((sector[dasect].ceilingz - s.z) >> 8) < r) {
					d = (int) (klabs(wall[sector[dasect].wallptr].x - s.x) + klabs(wall[sector[dasect].wallptr].y
							- s.y));
					if (d < r)
						checkhitceiling(dasect);
					else {
						d = (int) (klabs(wall[wall[wall[sector[dasect].wallptr].point2].point2].x
	...
	public static void movefx() {
		...
		while (i >= 0) {
			...
			case MUSICANDSFX:
				...
				ht = s.hitag;
				if (hittype[i].temp_data[1] != (cfg.SoundToggle ? 1 : 0)) {
					hittype[i].temp_data[1] = cfg.SoundToggle ? 1 : 0;
					hittype[i].temp_data[0] = 0;
				}
				...
				if (s.lotag >= 1000 && s.lotag < 2000) {
					...
					if ((soundm[s.lotag] & 2) != 0) {
						...
						if (x < ht && hittype[i].temp_data[0] == 0 && engine.getAudio().getSound().isAvailable(soundpr[s.lotag]-1))
						{
							if (numenvsnds == cfg.NumVoices) {
								j = headspritestat[11];
								while (j >= 0) {
	...
	public static void movefallers() {
		...
		while (i >= 0) {
			...
			if (hittype[i].temp_data[0] == 0) {
				...
				j = ifhitbyweapon(i);
				if (j >= 0) {
					if (j == CROSSBOW || j == RADIUSEXPLOSION || j == SEENINE
							|| j == 1273) {
						if (s.extra <= 0) {
							hittype[i].temp_data[0] = 1;
							j = headspritestat[12];
	...
	private static void Detonate(SPRITE s, int i, int[] t) {
		...
		if ((t[3] == 1 && s.xrepeat != 0) || s.lotag == -99) {
			int x = s.extra;
			spawn(i, EXPLOSION2);
			hitradius(i, seenineblastradius, x >> 2, x - (x >> 1),
					x - (x >> 2), x);
	...
	public static void movestandables() {
		...
		BOLT: for (; i >= 0; i = nexti) {
			...
			case DOORSHOCK:
				j = (int) (klabs(sector[sect].ceilingz - sector[sect].floorz) >> 9);
				s.yrepeat = (short) (j + 4);
				s.xrepeat = 16;
	...
	public static void movetransports() {
		...
		BOLT: for (; i >= 0; i = nexti) {
			...
			int nextj = j >= 0 ? nextspritesect[j] : 0;
				...
				case 10: // Player
					...
					if (sprite[j].owner != -1) {
						...
						if (onfloorz != 0
							...
							ps[p].oposz = ps[p].posz = sector[sprite[sprite[i].owner].sectnum].ceilingz
									+ (7 << 8);
						...
						if (onfloorz != 0
							...
							ps[p].oposz = ps[p].posz = sector[sprite[sprite[i].owner].sectnum].floorz
									- (7 << 8);
							...
							case WATERBUBBLE:
								if ((engine.krand() >> 8) < 63) {
									...
									case 2:
										sprite[j].x += (sprite[sprite[i].owner].x - sprite[i].x);
										sprite[j].y += (sprite[sprite[i].owner].y - sprite[i].y);
										sprite[j].z = sector[sprite[sprite[i].owner].sectnum].floorz
												- ll;
	...
	public static void moveactors() {
		...
		BOLT: for (; i >= 0; i = nexti) {
			...
			case STRIPEBALL:
				if (s.xvel != 0) {
					...
					if (clipmove_sectnum != -1) {
						...
						s.y = clipmove_y;
						s.z = clipmove_z;
						s.sectnum = (short) clipmove_sectnum;
					}
			...
			case 876:
				...
				if (hittype[i].temp_data[0] >= (respawnitemtime >> 1)
						&& hittype[i].temp_data[0] < ((respawnitemtime >> 1) + (respawnitemtime >> 2)))
					sprite[i].picnum = 876;
				makeitfall(i);
				break;
			...
			case REACTOR2:
				...
				if (t[1] != 0) {
					...
					case 3:
						...
						hitradius(i, 4096, impact_damage << 2,
	...
	public static void moveeffectors() // STATNUM 3
	{
		...
		BOLT: for (; i >= 0; i = nexti) {
			...
			case 14:
				...
				if (s.owner == -1) {
					dassert("Could not find any locators for SE# 6 and 14 with a hitag of " + t[3]);
				}
				...
				if (s.xvel != 0) {
					...
					if (s.xvel == sc.extra) {
						if ((sc.floorstat & 1) == 0
							...
							if (x < 20480) {
								j = s.ang;
								s.ang = (short) engine.getangle(s.x
			...
			case 5:
				...
				if (x < 8192) {
					j = s.ang;
					s.ang = (short) engine.getangle(s.x - ps[p].posx, s.y
							- ps[p].posy);
					shoot(i, FIRELASER);
					s.ang = (short) j;
				}
				...
				if (ldist(sprite[s.owner], s) < 1024) {
					short ta;
					ta = s.ang;
					s.ang = (short) engine.getangle(ps[p].posx - s.x,
							ps[p].posy - s.y);
					s.ang = ta;
					s.owner = -1;
					continue;
