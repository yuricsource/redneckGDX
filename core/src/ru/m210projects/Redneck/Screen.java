...
//		engine.rotatesprite(320 << 16, 166 << 16, 0x8000, 1024, WIDEHUD_PART2, 4, 0, 4 | 26 | 512, 0, 0, xdim - 1, ydim - 1);
	...
	public static void invennum(int x,int y,int num1,int ha, int sbits)
	{
	    ...
	    if(num1 > 99)
	    {
	        engine.rotatesprite(coordsConvertXScaled(x-4, type)<<16,coordsConvertYScaled(y)<<16,gViewYScaled>>1,0,THREEBYFIVE+dabuf[0]-'0',ha,0,sbits,0,0,xdim-1,ydim-1);
	        engine.rotatesprite(coordsConvertXScaled(x, type)<<16,coordsConvertYScaled(y)<<16,gViewYScaled>>1,0,THREEBYFIVE+dabuf[1]-'0',ha,0,sbits,0,0,xdim-1,ydim-1);
	        engine.rotatesprite(coordsConvertXScaled(x+4, type)<<16,coordsConvertYScaled(y)<<16,gViewYScaled>>1,0,THREEBYFIVE+dabuf[2]-'0',ha,0,sbits,0,0,xdim-1,ydim-1);
	    }
	    else if(num1 > 9)
	    {
	    	engine.rotatesprite(coordsConvertXScaled(x, type)<<16,coordsConvertYScaled(y)<<16,gViewYScaled>>1,0,THREEBYFIVE+dabuf[0]-'0',ha,0,sbits,0,0,xdim-1,ydim-1);
	    	engine.rotatesprite(coordsConvertXScaled(x+4, type)<<16,coordsConvertYScaled(y)<<16,gViewYScaled>>1,0,THREEBYFIVE+dabuf[1]-'0',ha,0,sbits,0,0,xdim-1,ydim-1);
	    }
	    else
	    	engine.rotatesprite(coordsConvertXScaled(x+4, type)<<16,coordsConvertYScaled(y)<<16,gViewYScaled>>1,0,THREEBYFIVE+dabuf[0]-'0',ha,0,sbits,0,0,xdim-1,ydim-1);
	}
