	...
	public void mPostDraw(MenuItem item) {
		...
		if ( item.isFocused() ) {
			...
			if(item instanceof MenuVariants) {
				engine.rotatesprite(160<<16,(item.y + tilesizy[BIGFNTCURSOR])<<16,8192,0,SPINNINGNUKEICON+((totalclock>>3)&15),8 - (totalclock & 0x3F),0,2|8,0,0,xdim-1,ydim-1);
			}
