...
public class RRSliderDrawable extends SliderDrawable {
    ...
    public void drawSliderBackground(int x, int y, int shade, int pal) {
        ...
        renderer.rotatesprite(x - 2 << 16, (int) ((y + 0.5f) * 65536.0f), 32768, 0, SLIDEBAR, 8, pal, 10 | 16);
    }
    ...
    public void drawSlider(int x, int y, int shade, int pal) {
        ...
        Renderer renderer = game.getRenderer();
        renderer.rotatesprite((int) ((x - 0.5f) * 65536.0f), y - 3 << 16, 24576, 0, 623, shade, pal, 10 | 16);
    }
    ...
    public void drawScrollerBackground(int x, int y, int height, int shade, int pal) {
        ...
        renderer.rotatesprite(x + 12 << 16, y - 7 << 16, 65536, ang, SLIDEBAR, 8, 0, 10 | 16, 0, 0, xdim - 1, coordsConvertYScaled(sy));

        ArtEntry pic = engine.getTile(SLIDEBAR);
        ...
        while (clen > 0) {
            ...
            renderer.rotatesprite(x + 12 << 16, (posy - 9) << 16, 65536, ang, SLIDEBAR, 8, 0, 10 | 16, 0, coordsConvertYScaled(posy), xdim - 1, coordsConvertYScaled(posy + dy));
            posy += dy;
            clen -= dy;
        }

        int y2 = sy + height - 15;
        renderer.rotatesprite(x + 12 << 16, (y2 - (pic.getWidth()) + 13) << 16, 65536, ang, SLIDEBAR, 8, 0, 10 | 16, 0, coordsConvertYScaled(y2), xdim - 1, ydim - 1);
    }

    @Override
    public void drawScroller(int x, int y, int shade, int pal) {
        Renderer renderer = game.getRenderer();
        renderer.rotatesprite((x + 14) << 16, (y) << 16, 32768, 512, 623, shade, 2, 10 | 16);
    }
