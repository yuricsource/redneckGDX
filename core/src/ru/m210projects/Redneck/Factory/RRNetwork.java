...
import static ru.m210projects.Redneck.RSector.ceilingspace;
import static ru.m210projects.Redneck.RSector.floorspace;
...
public class RRNetwork extends BuildNet {
    ...
    public final PLocation[] predictFifo = new PLocation[kNetFifoSize];
    ...
    public int GetPackets(byte[] p, int ptr, int len, final int nPlayer) {
        ...
        switch (p[ptr++]) {
            ...
            case kPacketContentRequest:
                ...
                try (ByteArrayInputStream is = new ByteArrayInputStream(p, ptr + 4, len)) {
                    ...
                    if (isAddon) {
                        ...
                        ini = findAddon(path, addonPackedIniName);
                    } else {
                        fil = game.getCache().getGameDirectory().getEntry(FileUtils.getPath(path));
                    }
                    ...
                    if (fil.exists() || ini != null) {
                        ...
                        if (ini != null) {
                            ...
                            if (mycrc == crc32) {
                                ...
                                found = 2;
                                Console.out.println("Player" + nPlayer + " - " + ud.user_name[nPlayer]
            ...
            case kPacketDisconnect:
                return GetDisconnectPacket(p, ptr, len, nPlayer, nDelete -> {
            ...
            case kPacketSound:
                ...
                if (cfg.isNoSound() || ud.lockout == 1 || Main.RTS_File == null || Main.RTS_File.getSize() == 0) {
                    break;
                }

                Entry entry = RTS_File.getEntry(packbuf[1]);
                if (entry.exists()) {
    ...
    public void UpdatePrediction(NetInput input) {
        ...
        int sb_snum;
        int psectlotag, tempsect, backcstat;
        boolean shrunk;
        int spritebridge;
        ...
        backcstat = psp.getCstat();
        psp.setCstat(psp.getCstat() & ~257);

        sb_snum = syn.bits;

        final int psect = predict.sectnum;
        Sector psec = boardService.getSector(psect);
        if (psec == null) {
            return;
        }

        psectlotag = psec.getLotag();
        spritebridge = 0;
        ...
        shrunk = (psp.getYrepeat() < 8);

        if (!ud.clipping && (psec.getFloorpicnum() == MIRROR)) {
        ...
        if (p.OnMotorcycle && psp.getExtra() > 0) {
            ...
            if (p.CarSpeed >= 20 && p.on_ground && (var4 || var5)) {
                ...
                if (p.CarVar5 != 0 || p.CarVar4 != 0 || p.NotOnWater == 0) {
                    ...
                    if (p.CarVar6 != 0) {
                        predict.xvel += (speed >> 5) * 16 * EngineUtils.sin((angvel + 512) & kAngleMask);
                        predict.yvel += (speed >> 5) * 16 * EngineUtils.sin(angvel & kAngleMask);
                        predict.ang = ((short) predict.ang - (dang >> 2)) & kAngleMask;
                    } else {
                        predict.xvel += (speed >> 7) * 16 * EngineUtils.sin((angvel + 512) & kAngleMask);
                        predict.yvel += (speed >> 7) * 16 * EngineUtils.sin(angvel & kAngleMask);
                        predict.ang = ((short) predict.ang - (dang >> 6)) & kAngleMask;
                    }
                } else if (p.CarVar6 != 0) {
                    predict.xvel += (p.CarSpeed >> 5) * 16 * EngineUtils.sin((angvel + 512) & kAngleMask);
                    predict.yvel += (p.CarSpeed >> 5) * 16 * EngineUtils.sin(angvel & kAngleMask);
                    predict.ang = ((short) predict.ang - (dang >> 4)) & kAngleMask;
                } else {
                    predict.xvel += (p.CarSpeed >> 7) * 16 * EngineUtils.sin((angvel + 512) & kAngleMask);
                    predict.yvel += (p.CarSpeed >> 7) * 16 * EngineUtils.sin(angvel & kAngleMask);
                    predict.ang = ((short) predict.ang - (dang >> 4)) & kAngleMask;
                }
            }
            ...
            if (p.CarSpeed > 0 && p.on_ground && (var5 || var6)) {
                ...
                int speed = 4 * p.CarSpeed;
                if (p.CarVar6 != 0) {
                    predict.xvel += (speed >> 6) * 16 * EngineUtils.sin((angvel + 512) & 0x7FF);
                    predict.yvel += (speed >> 6) * 16 * EngineUtils.sin(angvel & 0x7FF);
                    predict.ang = ((short) predict.ang - (dang >> 5)) & kAngleMask;
                } else {
                    predict.xvel += (speed >> 7) * 16 * EngineUtils.sin((angvel + 512) & 0x7FF);
                    predict.yvel += (speed >> 7) * 16 * EngineUtils.sin(angvel & 0x7FF);
                    predict.ang = ((short) predict.ang - (dang >> 6)) & kAngleMask;
                }
            }
        }
        ...
        if ((lz & kHitSprite) == kHitSector && psectlotag == 1 && klabs(predict.z - j) > PHEIGHT + (16 << 8)) {
            psectlotag = 0;
        }

        if (p.aim_mode == 0 && predict.onground && psectlotag != 2 && (psec.getFloorstat() & 2) != 0) {
        ...
        if ((lz & kHitTypeMask) == kHitSprite) {
            ...
            if (hsp != null) {
                if ((hsp.getCstat() & 33) == 33) {
                    psectlotag = 0;
                    spritebridge = 1;
                }
                if (badguy(hsp) && hsp.getXrepeat() > 24 && klabs(psp.getZ() - hsp.getZ()) < (84 << 8)) {
                    j = EngineUtils.getAngle(hsp.getX() - predict.x, hsp.getY() - predict.y);
                    predict.xvel -= EngineUtils.sin((j + 512) & 2047) << 4;
                    predict.yvel -= EngineUtils.sin(j & 2047) << 4;
                }
        ...
        if (psp.getExtra() <= 0) {
            ...
            int sect = engine.updatesector(predict.x, predict.y, predict.sectnum);
            if (sect != -1) {
        ...
        if (p.on_crane < 0) {
            ...
            if (psectlotag == 2) {
                ...
                {
                    if ((sb_snum & 3) == 0 && predict.onground && (psec.getFloorstat() & 2) != 0
                    ...
                    if (psectlotag != 1 && !predict.onground && predict.zvel > 6144 >> 1) {
                ...
                if (predict.jumpingcounter != 0) {
                    ...
                    if (predict.jumpingcounter < (768)) {
                        predict.zvel -= (EngineUtils.sin((2048 - 128 + predict.jumpingcounter) & 2047)) / 12;
        ...
        if ((j & kHitTypeMask) == kHitWall) {
            ...
            if (hwp != null && !p.OnMotorcycle && !p.OnBoat) {
                if (hwp.getLotag() >= 40 && hwp.getLotag() <= 44) {
    ...
    public void CalcChecksum() {
        if ((numplayers >= 2 || mFakeMultiplayer) && ((gNetFifoTail & 7) == 7)) // build sync variables
        {
            ...
            for (int i = connecthead; i >= 0; i = connectpoint2[i]) {
                ...
                gChecksum[1] ^= (int) checksum(ps[i]);
                if (psp != null) {
                    gChecksum[2] ^= (int) checksum(psp);
                }
            }
        ...
        if (PlayerSyncRequest != -1) {
            ...
            Sprite psp = boardService.getSprite(ps[pnum].i);

            Console.out.println("Player: " + pnum);
            Console.out.println(ps[pnum].toString());
            if (psp != null) {
                Console.out.println("Sprite: ");
                Console.out.println(psp.toString());
            }
            Console.out.println("gNetFifoTail: " + gNetFifoTail);
    ...
    public boolean WaitForContentCheck(byte[] contentPacket, int timeout) {
        ...
        System.arraycopy(contentPacket, 0, packbuf, 5, len);
    ...
    public void getnames() {
        ...
        if (numplayers > 1) {
            ...
            for (i = 0; i < cfg.getpName().length() && name[i] != 0; i++) {
                buf[l++] = (byte) name[i];
            }
            buf[l++] = 0;
